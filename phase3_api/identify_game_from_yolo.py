    ...
    try:
        ...
        with pipeline.driver.session() as session:
            query = """
                ...
                // Similaires par période (score 2)
                ...
                // Similaires par catégorie (score 1)
                ...
                // Combiner, dédoublonner en gardant le score le plus élevé
            ...
            return {
                ...
                "game": game_name,
                "similar_games": [],
                "message": "Aucun jeu similaire trouvé"
            }
    except Exception as e:
        raise HTTPException(500, f"Error: {str(e)}")
...
async def similar_games_by_type(
    game_name: str = Query(..., description="Nom du jeu"),
    similarity_type: str = Query("all", description="Type de similarité : region, period, category, all"),
    limit: int = Query(5)
):
    ...
    try:
        ...
        with pipeline.driver.session() as session:
            ...
            if similarity_type in ["region", "all"]:
                query_region = """
                    MATCH (g:Game) WHERE toLower(g.name) = toLower($name)
                    MATCH (g)-[:FROM_REGION]->(r:Region)<-[:FROM_REGION]-(g2:Game)
                    WHERE g2.name <> g.name
                    RETURN g2.name AS game, 'Région' AS type, r.name AS value
                    LIMIT $limit
                """
            
            if similarity_type in ["period", "all"]:
                query_period = """
                    MATCH (g:Game) WHERE toLower(g.name) = toLower($name)
                    MATCH (g)-[:FROM_PERIOD]->(p:Period)<-[:FROM_PERIOD]-(g2:Game)
                    WHERE g2.name <> g.name
                    RETURN g2.name AS game, 'Période' AS type, p.name AS value
                    LIMIT $limit
                """
            
            if similarity_type in ["category", "all"]:
                query_category = """
                    MATCH (g:Game) WHERE toLower(g.name) = toLower($name)
                    MATCH (g)-[:IN_CATEGORY]->(c:Category)<-[:IN_CATEGORY]-(g2:Game)
                    WHERE g2.name <> g.name
                    RETURN g2.name AS game, 'Catégorie' AS type, c.name AS value
                    LIMIT $limit
                """
        ...
        with pipeline.driver.session() as session:
            ...
            query = """
                ...
                WITH g, sig, r, p,
                     ...
                     END AS pieceScore,
                     CASE
                       WHEN sig.required_pieces IS NOT NULL 
                            AND size(sig.required_pieces) > 0 
                            AND size($detected) > 0
                       THEN toFloat(size([x IN sig.required_pieces WHERE x IN $detected])) / size(sig.required_pieces)
                       ELSE 0
                     END AS typeRatio
                WHERE pieceScore > 0 OR typeRatio > 0
                RETURN g.name AS game, g.description AS desc, g.ludCode AS ludCode,
                       g.origin AS origin, g.mappedRulesetId AS rulesetId,
                       sig.board_cols AS actual_cols, sig.board_rows AS actual_rows,
            ...
            result = session.run(query, 
                ...
                detected=detected_types, tolerance=tolerance
            ).data()
            
            if not result:
                raise HTTPException(404, "Aucun jeu identifié avec ces caractéristiques")
            ...
            # Générer le plateau
            board_layout = generate_board_layout(
                game["actual_cols"], 
                game["actual_rows"], 
                game.get("pieceTypes", []),
                game.get("expectedPieces")
            )
            ...
            return {
                ...
                "reconstruction": {
                    "board_dimensions": {
                        "cols": game["actual_cols"],
                        "rows": game["actual_rows"],
                        "total_cells": game["actual_cols"] * game["actual_rows"]
                    },
                    "pieces": {
                        "types": game.get("pieceTypes", []),
                        "total_expected": game.get("expectedPieces"),
                        "detected": len(request.pieces),
                        "missing": (game.get("expectedPieces") or 0) - len(request.pieces) if game.get("expectedPieces") else "inconnu"
                    },
                    "board_layout": board_layout,
                    "rules": {
                        "summary": game["desc"],
                        "detailed_rules": rules_text[:2000] if rules_text else "Règles non disponibles"
                    },
                    "historical_context": {
                        "regions": game.get("regions", []),
...
# ========== FONCTIONS UTILITAIRES POUR LA RECONSTRUCTION ==========
...
def extract_rules_from_lud(lud_code: str) -> str:
    ...
    # Extraire les descriptions entre guillemets
    import re
    ...
    for desc in descriptions:
        if len(desc) > 20 and not desc.startswith('{') and not desc.startswith('('):
            ...
            if len(rules) >= 5:  # Limiter à 5 descriptions
                break
    
    # Détecter les mécaniques de jeu
    if "Checkmate" in lud_code or "checkmate" in lud_code.lower():
        rules.append("Objectif : Mettre le roi adverse en échec et mat (Checkmate)")
    if "Stalemate" in lud_code or "stalemate" in lud_code.lower():
        rules.append("Stalemate : Match nul si aucun mouvement légal possible")
    if "capture" in lud_code.lower() or "remove" in lud_code.lower():
        rules.append("Capture : Les pièces peuvent être capturées")
    if "Promotion" in lud_code:
        rules.append("Promotion : Les pions peuvent être promus")
    if "Castling" in lud_code or "Kingside" in lud_code or "Queenside" in lud_code:
        rules.append("Roque : Mouvement spécial Roi + Tour")
    
    if not rules:
        rules.append("Règles complètes disponibles dans le code source .lud")
    
    return "\n".join(rules)


def generate_board_layout(cols: int, rows: int, piece_types: list, total_pieces: int) -> dict:
    """Génère une représentation du plateau"""
    layout = {
        "type": "grid",
        "dimensions": f"{cols}×{rows}",
        "total_cells": cols * rows,
        "ascii_representation": generate_ascii_board(cols, rows, piece_types)
    }
    
    # Suggérer la disposition pour les échecs
    if cols == 8 and rows == 8 and "pawn" in piece_types:
        layout["suggested_setup"] = {
            "white_pieces": "♜ ♞ ♝ ♛ ♚ ♝ ♞ ♜ (rangée 1) + 8 ♟ (rangée 2)",
            "black_pieces": "♜ ♞ ♝ ♛ ♚ ♝ ♞ ♜ (rangée 8) + 8 ♟ (rangée 7)"
        }
    elif cols == 3 and rows == 10:
        layout["suggested_setup"] = "Parcours en zigzag : 10 cases par rangée, 3 rangées"
    
    return layout


def generate_ascii_board(cols: int, rows: int, piece_types: list) -> str:
    """Génère une représentation ASCII simplifiée du plateau"""
    if cols > 20 or rows > 20:
        return f"[Plateau {cols}×{rows} - trop grand pour l'affichage]"
    
    # Utiliser des symboles Unicode pour les pièces
    piece_symbols = {
        "king": "♚", "queen": "♛", "rook": "♜",
        "bishop": "♝", "knight": "♞", "pawn": "♟",
        "dog": "🐕", "jaguar": "🐆"
    ...
    # Déterminer les pièces à afficher
    display_pieces = []
    for p in piece_types:
        if p.lower() in piece_symbols:
            display_pieces.append(piece_symbols[p.lower()])
    
    if not display_pieces:
        display_pieces = ["●"]
    ...
    for row in range(rows):
        lines.append("|" + "   |" * cols)
        # Afficher des pièces sur la première et dernière rangée
        if row == 0 and len(display_pieces) > 0:
            pieces_row = "|"
            for col in range(cols):
                symbol = display_pieces[col % len(display_pieces)]
                pieces_row += f" {symbol} |"
            lines.append(pieces_row)
        elif row == rows - 1 and len(display_pieces) > 0:
            pieces_row = "|"
            for col in range(cols):
                symbol = display_pieces[(col + 1) % len(display_pieces)]
                pieces_row += f" {symbol} |"
            lines.append(pieces_row)
        else:
            lines.append("|" + "   |" * cols)
